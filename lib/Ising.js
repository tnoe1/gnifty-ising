...
class Ising {
    ...
    /**
     * Compute the change in energy caused by simultaneous spin flips at given
     * indices. Does not mutate the configuration state.
     * 
     * @param i The row of the particle whose spin gets changed
     * @param j The column of the particle whose spin gets changed
     */
    batch_induced_delta_e(indices) {
        let current_energy = this.current_energy();

        indices.forEach((i) => {
            this.toggle_spin(i[0], i[1]);
        });

        let induced_energy = this.current_energy();

        indices.forEach((i) => {
            this.toggle_spin(i[0], i[1]);
        });

        return induced_energy - current_energy;
    }
    ...
    // compute_experimental_distribution({ num_sweeps = 1000 }={}) {
    //     let data = [];
    //     for (let i = 0; i < num_sweeps; i++) {
    //         data.push(this.run_sweep());
    //     }

    //     let hist = new Array(2**this.N).fill(0);
    //     data.forEach((d) => {
    //         hist[d.encoding] += 1;
    //     });
    ...
    get_color_energy_delta(color) {
        let indices = this._get_color_indices(color);
        return this.batch_induced_delta_e(indices);
    }
}
